...
1. Отладить `main.c` на железе: убедиться, что режим `drone` работает до появления Циклопа, а после первого байта USART1 включается режим `cyclops`.
2. Настроить и проверить таймеры на heartbeat/status LED.
3. Отладить оптопары: убедиться, что они открываются когда надо.
4. Проверить работу с дроном: `prepare`, `cancel`, `fly`, реакции на `SAFETY`, `LAUNCH`, `END`.
5. Довести файлы протоколов: `mavlink_rx`, `mavlink_tx`, `cyclops_rx`, `cyclops_tx`.
6. Написать полноценную логику Циклопа в `logic_cyclops`.
